...
#include <stdlib.h>
#include <string.h>
/* CMSIS-RTOS types and APIs (osMutexId_t, osOK, osMutexAcquire, etc.) */
#include "cmsis_os.h"
...
/* Mutex protecting shared signal-related globals (defined in main.c) */
extt osMutexId_t SignalMutexHandle;
/** @brief DIP switch status register - holds state of 12 DIP switches (bits 0-11).
 *         Changed from uint8_t to uint16_t to prevent bit shift overflow.
 *         PROTECTED by SignalMutexHandle - always acquire mutex before reading in concurrent contexts */
...
extt volatile uint8_t u8IsFirstSnycArrived;	   /* PROTECTED by SignalMutexHandle */
extt volatile uint8_t u8IsInTp20Measurement;   /* PROTECTED by SignalMutexHandle */
extt volatile uint16_t u161MilliSecondCounter; /* Read-only safe; written by timer ISR */
